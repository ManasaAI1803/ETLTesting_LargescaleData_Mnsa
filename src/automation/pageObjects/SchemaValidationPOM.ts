import geSchemaValidator, {
  SchemaComparisonResult,
  SchemaValidationResult,
} from '../../utils/geSchemaValidator';
 ...
 */
export class SchemaValidationPOM {
  /**
   ...
   */
  async validateTable(tableName: string, data: any[]): Promise<SchemaValidationResult> {
    ...
    const result = geSchemaValidator.validateSchema(tableName, data);
    return result;
  }
  ...
  async compareTableSchemas(
    ...
    try {
      const result = await geSchemaValidator.compareSchemas(tableName, sourceData, targetData);
      ...
      if (!result || !result.source_schema || !result.target_schema) {
        ...
        return {
          ...
          schema_compatible: result?.schema_compatible || false,
          differences: result?.differences || ['Schema comparison incomplete'],
   ...
   */
  async hasValidSchema(tableName: string, data: any[]): Promise<boolean> {
    const result = await this.validateTable(tableName, data);
   ...
   */
  formatValidationErrors(result: SchemaValidationResult): string {
  ...
  private validateTableName(tableName: string): void {
    ...
    if (!supportedTables.includes(tableName)) {
      throw new Error(
        `Unsupported table: ${tableName}. Supported tables: ${supportedTables.join(', ')}`
      );
    }
  }
}

export default new SchemaValidationPOM();
