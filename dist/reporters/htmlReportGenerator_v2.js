...
exports.HtmlReportGeneratorV2 = void 0;
...
class HtmlReportGeneratorV2 {
    ...
    generateHtmlReport() {
        ...
        const queryCount = (_c = (_b = (_a = this.queryLog) === null || _a === void 0 ? void 0 : _a.queries) === null || _b === void 0 ? void 0 : _b.length) !== null && _c !== void 0 ? _c : 0;
...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            const sourceCount = (_b = (_a = context === null || context === void 0 ? void 0 : context.sourceData.length) !== null && _a !== void 0 ? _a : issue.sourceCount) !== null && _b !== void 0 ? _b : 0;
            const targetCount = (_d = (_c = context === null || context === void 0 ? void 0 : context.targetData.length) !== null && _c !== void 0 ? _c : issue.targetCount) !== null && _d !== void 0 ? _d : 0;
    ...
    getBusinessRule(result) {
        var _a;
        if (result.businessRulesApplied && result.businessRulesApplied.length > 0) {
            return result.businessRulesApplied.join(', ');
        }
        const text = `${result.test} ${((_a = result.steps) === null || _a === void 0 ? void 0 : _a.map(step => step.log).join(' ')) || ''}`.toLowerCase();
        if (/schema|schema mismatch|fields?/.test(text))
            return 'Schema Validation';
        if (/row count|count mismatch/.test(text))
            return 'Row Count Validation';
        if (/hash|data hash|hash mismatch/.test(text))
            return 'Hash Validation';
        if (/dependency|referential|visit_id|patient_id|provider_id|billing.*visit|providers/.test(text))
            return 'Dependencies Validation';
        if (/amount|billing|negative|decimal|precision|truncation/.test(text))
            return 'Amount Validation';
        if (/missing|extra|diff|partition|anti|left join|right join/.test(text))
    ...
    generateRulesSection() {
        ...
        return this.contexts.map(context => {
            const rules = businessRules_1.businessRules[context.table];
            const mandatoryFields = (rules === null || rules === void 0 ? void 0 : rules.mandatoryFields) || [];
            const acceptableValues = (rules === null || rules === void 0 ? void 0 : rules.acceptableValues) || {};
            const validationFields = (rules === null || rules === void 0 ? void 0 : rules.validationRules) ? Object.keys(rules.validationRules) : [];
            return `
                <div class="anomaly-table">
    ...
    calculateSchemaIssueSummary(tableIssues) {
    ...
    getRulesSummary(tableName) {
        const key = tableName.toLowerCase();
        const rules = businessRules_1.businessRules[key];
        const mandatoryFields = (rules === null || rules === void 0 ? void 0 : rules.mandatoryFields) || [];
        const validationFields = (rules === null || rules === void 0 ? void 0 : rules.validationRules) ? Object.keys(rules.validationRules) : [];
        const acceptableValues = (rules === null || rules === void 0 ? void 0 : rules.acceptableValues) ? Object.keys(rules.acceptableValues) : [];
        const parts = [];
    ...
    extractSchemaValidationIssues() {
        const schemaIssues = new Map();
    ...
    extractTableNameFromTest(testName) {
    ...
    mergeTableIssues(schemaIssuesByTable) {
        const merged = new Map(this.contexts.map(context => [context.table.toLowerCase(), { table: context.table, count: context.anomalies.length }]));
    ...
    buildTableIssueRows(schemaIssuesByTable) {
    ...
    calculateMatchPercentFromCounts(sourceCount, targetCount) {
        if (!sourceCount || !targetCount) {
            return 0;
        }
        return sourceCount === targetCount ? 100 : (Math.min(sourceCount, targetCount) / Math.max(sourceCount, targetCount)) * 100;
...
exports.HtmlReportGeneratorV2 = HtmlReportGeneratorV2;
